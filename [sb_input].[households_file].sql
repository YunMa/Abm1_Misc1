...
/****** Object:  UserDefinedFunction [sb_input].[households_file]    Script Date: 4/23/2019 9:04:43 PM ******/
SET ANSI_NULLS ON
GO
...
(
	...
	[base_and_new_popsyn_run_id] [smallint] NOT NULL,
...
SELECT
	...
	,[synpop_hh].[popsyn_run_id] AS [base_and_new_popsyn_run_id]
	,[n]
	,[final_weight]
	,[synpop_hh_id]
FROM
...
WHERE
	[synpop_hh].[popsyn_run_id] IN (@base_popsyn_run_id, @popsyn_run_id)
...
order by  [base_and_new_popsyn_run_id],[synpop_hh_id],n       --added on 10/02/15, YMA
RETURN
END
...
EXEC sys.sp_addextendedproperty @name=N'ms_description', @value=N'function to output households input file for the ABM model for service bureau jobs' , @level0type=N'SCHEMA',@level0name=N'sb_input', @level1type=N'FUNCTION',@level1name=N'households_file'
GO

EXEC sys.sp_addextendedproperty @name=N'subsystem', @value=N'sb_input' , @level0type=N'SCHEMA',@level0name=N'sb_input', @level1type=N'FUNCTION',@level1name=N'households_file'
GO
