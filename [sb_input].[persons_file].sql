...
/****** Object:  UserDefinedFunction [sb_input].[persons_file]    Script Date: 4/23/2019 9:05:06 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE FUNCTION
	[sb_input].[persons_file]
(
	@base_popsyn_run_id smallint
...
INNER JOIN
	[sb_input].[households_file](@base_popsyn_run_id, @popsyn_run_id) as hh_file --changed on 10/02/15, though it may not be necessary
ON
	[synpop_person].[popsyn_run_id] = hh_file.[base_and_new_popsyn_run_id]
	AND [synpop_person].[synpop_hh_id] = hh_file.[synpop_hh_id]
	AND [expansion_numbers].[n] = hh_file.[n]
WHERE
	[synpop_person].[popsyn_run_id] IN (@base_popsyn_run_id, @popsyn_run_id)
	AND NOT ([synpop_hh].[popsyn_run_id] = @base_popsyn_run_id AND [synpop_hh].[mgra] IN (SELECT DISTINCT [mgra] FROM [popsyn].[synpop_hh] WHERE [popsyn_run_id] = @popsyn_run_id))
ORDER BY
...
EXEC sys.sp_addextendedproperty @name=N'ms_description', @value=N'function to output persons input file for ABM for service bureau projects' , @level0type=N'SCHEMA',@level0name=N'sb_input', @level1type=N'FUNCTION',@level1name=N'persons_file'
GO

EXEC sys.sp_addextendedproperty @name=N'subsystem', @value=N'sb_input' , @level0type=N'SCHEMA',@level0name=N'sb_input', @level1type=N'FUNCTION',@level1name=N'persons_file'
GO
